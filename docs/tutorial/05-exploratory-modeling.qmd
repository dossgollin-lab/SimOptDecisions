...
function SimOptDecisions.run_timestep(
    ...
    surge_dist = GeneralizedExtremeValue(scenario.gev_μ, scenario.gev_σ, scenario.gev_ξ)
    ...
    damage_fraction = depth_damage(flood_depth_at_house, scenario.dd_threshold, scenario.dd_saturation)
...
function SimOptDecisions.compute_outcome(
    ...
    npv_damages = sum(
        step_records[t].damage_fraction * discount_factor(scenario.discount_rate, t)
...
let
    ...
    for rate in rate_bins
        filtered_scenarios = filter(s -> abs(s.discount_rate - rate) < 0.005, scenarios)
...
To use `explore()`, wrap your Scenario, Policy, and Outcome fields with parameter types:
...
println("Columns: policy_idx, scenario_idx, plus flattened parameters and outcomes")
```
...
println("  Discount rate: $(round(r.scenario_discount_rate, digits=3))")
