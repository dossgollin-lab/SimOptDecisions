...
| `TimeSeriesParameter{T,I}` | Time series | `TimeSeriesParameter(2020:2030, values)` |
| `GenericParameter{T}` | Complex objects | `GenericParameter(my_object)` |

::: {.callout-note}
`GenericParameter` is for complex objects that don't fit other types. These fields are **skipped** during exploration (not flattened to columns). Prefer `CategoricalParameter` with an identifier when possible.
:::
