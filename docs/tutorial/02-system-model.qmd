...
- `water_level` is an **input** (from the scenario), not generated here
- All arguments are positional for performance (no keyword argument overhead)
- We use `T<:AbstractFloat` for type stability—Julia infers `T` from the arguments
- The function is pure physics: inputs → outputs
...
Construction cost increases with elevation. Damage decreases—but not linearly. The optimal choice isn't obvious, and becomes even less obvious when we consider:

1. **Multiple years**: Damages accumulate over time
2. **Discounting**: Future damages are worth less than present costs
3. **Uncertainty**: The water level distribution parameters are themselves uncertain
