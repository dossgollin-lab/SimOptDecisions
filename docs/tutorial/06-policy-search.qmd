...
# Required for optimization: vector constructor and type-level param_bounds
...
The optimizer requires two things beyond `simulate()`:

1. A **vector constructor**: `MyPolicy(params::AbstractVector)` that creates a policy from a flat parameter vector
2. A **type-level `param_bounds`**: `param_bounds(::Type{<:MyPolicy})` returning `[(lo, hi), ...]`

`params(policy)` (extracting current values from a policy instance) is auto-derived from `ContinuousParameter` fields.
