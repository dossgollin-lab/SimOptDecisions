...
# Required for optimization: vector constructor (params/param_bounds are auto-derived from @policydef)
...
The optimizer requires one thing beyond `simulate()`:

- A **vector constructor**: `MyPolicy(params::AbstractVector)` that creates a policy from a flat parameter vector.

Two other interface methods—`params(policy)` and `param_bounds(policy)`—are **auto-derived** from `@policydef` fields.
Since `@policydef` stores bounds in `ContinuousParameter` wrappers, the framework can extract them automatically.
You can also pass explicit `bounds` to `optimize()`, which is what we do below to restrict the search range.
