...
`SharedParameters` wraps fixed parameters that are constant across all scenarios and not subject to optimization. Use it for things like discount rates, planning horizons, and physical constants.
...
exp_config = ExperimentConfig(
    ...
    scenario_source="Latin Hypercube sampling from priors"
)
```
...
| `scenarios` | `Vector{<:AbstractScenario}` | The scenarios used in this experiment |
...
scenarios = [sample_scenario(rng) for _ in 1:1000]
...
exp_config = ExperimentConfig(
    seed, scenarios, shared, backend;
    git_commit=read(`git rev-parse HEAD`, String) |> strip,
    scenario_source="1000 scenarios from Latin Hypercube sampling"
)

# 2. Run optimization
prob = OptimizationProblem(config, scenarios, MyPolicy, calculate_metrics, objectives)
result = SimOptDecisions.optimize(prob, backend)
