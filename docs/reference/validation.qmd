...
SimOptDecisions can validate that your Scenario, Policy, and Outcome types use parameter wrappers (`ContinuousParameter`, `DiscreteParameter`, `CategoricalParameter`, `TimeSeriesParameter`, or `GenericParameter`).

### When Is Validation Required?

| Function | Parameter Types | Notes |
|----------|-----------------|-------|
| `simulate()` | Optional (by default) | Plain types work fine |
| `explore()` | **Required** | Always validates; will error if types aren't wrapped |
| `optimize()` | Recommended | Enables automatic `param_bounds` derivation |

### Enabling Strict Validation

To validate parameter types during `simulate()`, set the environment variable:

```julia
ENV["SIMOPT_STRICT_VALIDATION"] = "true"
```

When enabled, `simulate()` will check that all Scenario, Policy, and Outcome fields use parameter wrappers and throw a `ParameterTypeError` if they don't.
...
This catches type errors early, before you try to use `explore()` or optimization.
:::
