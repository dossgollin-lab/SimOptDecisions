...
Sequential decision problems typically follow a consistent formulation:
...
This package is inspired by this theoretical framework, as well as by literature on multi-objective optimization and decision-making under deep uncertainty.

We use a **simulation-optimization** framework to design parametric policies that cannot be solved exactly, allowing us to study highly complex, nonlinear, or black-box transition functions (i.e., models) and develop useful decision-support tools.
It is particularly motivated by the author's work on climate-risk assessment and management, but is applicable to many types of problems.
...
You don't need to use all the loops. Pick the level that fits your task:

| Function | Purpose |
|----------|---------|
| `simulate(config, sow, policy)` | Run one scenario — explore dynamics, debug |
| `evaluate_policy(prob, policy)` | Assess a policy across many SOWs — no optimization |
| `optimize(prob, backend)` | Search for optimal policy parameters |

Start with `simulate` to understand your model, then scale up.
:::
