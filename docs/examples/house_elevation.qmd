...
The policy has a single continuous parameter: the elevation height in feet.
The optimizer explores `[0, 14]` continuously; after optimization, we discretize the Pareto front to practical values (0 or 3–14 ft).

::: {.callout-tip}
## Continuous Relaxation for Discrete Choices

Real elevation choices are discrete: you either don't elevate (0 ft) or elevate to at least 3 ft (the minimum practical height per USACE).
Rather than encoding this as a constraint (which creates discontinuities that confuse convergence metrics), we:

1. **Optimize continuously** over `[0, 14]` — the optimizer sees a smooth landscape
2. **Discretize after optimization** — snap Pareto solutions to valid values and re-evaluate

This preserves smooth convergence behavior while still producing practical recommendations.
:::

```{julia}
#| output: false
struct ElevationPolicy{T<:AbstractFloat} <: AbstractPolicy
    elevation_ft::T  # continuous: 0-14 ft during optimization
end

# For optimization: convert between policy and parameter vector
...
```{julia}
#| output: false
# Define the action: directly use the policy's elevation parameter
...
The base cost phases in over the first foot to create a smooth function
for continuous optimization (avoids discontinuity at Δh=0).
"""
function elevation_cost(Δh::Real, area_ft2::Real, house_value::Real)
    ...
    full_base_cost = 10_000 + 300 + 470 + 4_300 + 2_175 + 3_500  # = $20,745

    # Phase in base cost over first foot (creates smooth transition at Δh=0)
    # This reflects that very small elevations aren't practical
    base_cost = full_base_cost * min(Δh, 1.0)
    ...
    cost_dollars = base_cost + area_ft2 * rate * Δh
...
The optimizer explored continuous elevations, but real decisions must be discrete.
We snap Pareto solutions to practical values and re-evaluate:

```{julia}
#| output: false
"""
Discretize a continuous elevation to practical values.
Returns 0 if below threshold, otherwise rounds to nearest integer ≥ 3.
"""
function discretize_elevation(elev::Real; threshold=1.5, min_elevation=3)
    elev < threshold && return 0.0
    return max(min_elevation, round(Int, elev)) |> Float64
end

# Collect and discretize Pareto solutions
pareto_pts = collect(SimOptDecisions.pareto_front(opt_result))
discrete_policies = unique([
    ElevationPolicy(discretize_elevation(p[1]))
    for (p, _) in pareto_pts
])

# Re-evaluate discretized policies to get accurate metrics
discrete_results = []
for policy in discrete_policies
    outcomes = [simulate(config, sow, policy) for sow in sows]
    metrics = calculate_metrics(outcomes)
    push!(discrete_results, (
        elevation=policy.elevation_ft,
        construction_cost=metrics.construction_cost,
        expected_damage=metrics.expected_damage,
    ))
end
```

```{julia}
#| label: fig-discrete-pareto
#| fig-cap: "Discretized Pareto front with practical elevation values (0 or 3-14 ft)"
#| code-fold: true
let
    ...
    ax = Axis(
        ...
        xlabel="Construction cost (fraction of house value)",
        ylabel="Expected NPV damages (fraction of house value)",
        title="Discretized Recommendations",
    )

    costs = [r.construction_cost for r in discrete_results]
    damages = [r.expected_damage for r in discrete_results]
    elevs = [r.elevation for r in discrete_results]

    scatter!(ax, costs, damages; markersize=15, color=:steelblue)

    for (i, elev) in enumerate(elevs)
        text!(ax, costs[i], damages[i];
            text="$(Int(elev)) ft", align=(:left, :bottom), offset=(5, 5))
    end
...
The discretized results give actionable recommendations: elevate to 0 ft (don't elevate), 3 ft, 4 ft, etc.
Each point represents a distinct practical choice.
...
5. **Continuous relaxation for discrete choices**: When your problem has discrete options (like "don't elevate" vs "elevate 3+ ft"), optimize continuously and discretize after. This preserves smooth convergence behavior while producing practical recommendations.
