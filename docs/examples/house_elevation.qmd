...
function SimOptDecisions.simulate(
    ...
    _, annual_damages =
        SimOptDecisions.Utils.run_timesteps(nothing, 1:(params.horizon)) do state, ts
            ...
            flood_depth_at_house =
                surge_at_gauge - (params.gauge_to_house + policy.elevation_ft)

            # Compute fractional damage this year
            damage_fraction = depth_damage(
                flood_depth_at_house, sow.dd_threshold, sow.dd_saturation
            )
    ...
    npv_damages = sum(
        annual_damages[t] * SimOptDecisions.Utils.discount_factor(sow.discount_rate, t) for
        t in eachindex(annual_damages)
    )
