...
```{julia}
#| output: false
Base.@kwdef struct HouseElevationParams{T <: AbstractFloat} <: AbstractFixedParams
...
Now we implement the core `simulate` function.
For each simulation, we loop through years, sample storm surges, compute damages, and then discount at the end.
...
function SimOptDecisions.simulate(
	...
	# Use run_timesteps to handle the loop with preallocation
	# This model has no state between years, so we pass `nothing`
	_, annual_damages = SimOptDecisions.Utils.run_timesteps(nothing, 1:params.horizon) do state, ts
		...
		# Convert to depth at house floor (accounting for elevation)
		...
		flood_depth_at_house = surge_at_gauge - (params.gauge_to_house + policy.elevation_ft)

		# Compute fractional damage this year
		damage_fraction = depth_damage(flood_depth_at_house, sow.dd_threshold, sow.dd_saturation)

		return (state, damage_fraction)  # state unchanged, output is damage
	end

	# Discount damages at the end (cleaner than accumulating during simulation)
	npv_damages = sum(
		annual_damages[t] * SimOptDecisions.Utils.discount_factor(sow.discount_rate, t)
		for t in eachindex(annual_damages)
	)

	return (npv_damages = npv_damages, annual_damages = annual_damages)
end
```

The `Utils.run_timesteps` helper handles the loop with preallocation—no `push!` needed.
The step function returns `(new_state, output)`, and all outputs are collected into a vector.
The `Utils.discount_factor` helper handles the discounting calculation at the end.
...
let
	...
	for elev in elevations
		...
		# Box (25-75)
		poly!(ax,
			Point2f[(elev-0.3, q25), (elev+0.3, q25), (elev+0.3, q75), (elev-0.3, q75)];
...
- **Outcome**: The result of a *single* simulation (one policy × one SOW). This is what `simulate` returns—in our case, a `NamedTuple` with `npv_damages` and `annual_damages`.
...
Note: construction_cost is computed from the policy elevation, not from outcomes.
We pass it through the metric calculator by extracting from the first outcome's context.
"""
function calculate_metrics(outcomes, policy)
	damages = [o.npv_damages for o in outcomes]
	# Construction cost depends only on the policy, not the SOW outcomes
	construction = elevation_cost(policy.elevation_ft)
	return (
...
println("Objectives: construction_cost = $(round(opt_result.best_objectives[1], digits=3)), ",
...
- Implementing the simulation with a simple for-loop
- Using `Utils.timeindex` and `Utils.discount_factor` helpers
