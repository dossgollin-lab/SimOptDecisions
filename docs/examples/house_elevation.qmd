...
# Note: We optimize over [1, 14] ft. The "don't elevate" option (0 ft) is a distinct
# choice that should be evaluated separately - you either elevate or you don't.
...
# The backend stores algorithm parameters; options is for advanced algorithm-specific settings
backend = MetaheuristicsBackend(; algorithm=:NSGA2, max_iterations=100, population_size=30)
```
...
- Defining the core types: Config, SOW, Action, Policy
- Using the `TimeStepping` interface with five callbacks: `initialize`, `get_action`, `run_timestep`, `time_axis`, and `finalize`
