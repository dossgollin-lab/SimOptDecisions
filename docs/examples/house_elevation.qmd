...
function SimOptDecisions.get_action(
    ...
    state::SeaLevelState,  # unused for static policy, but required
...
We define heights relative to a reference datum (e.g., NAVD88). This structure allows future extensions to include sea-level rise uncertainty.
...
In this example, MSL is constant over time. However, the state structure allows future extensions where MSL could vary—for example, sampling sea-level rise trajectories from uncertain projections in the SOW. This demonstrates the pattern of explicit state even for "nearly stateless" models.
:::
...
1. `initialize` - Create initial state (SeaLevelState with MSL)
...
We initialize the state with the current mean sea level. In this example, MSL starts at 0 (the reference datum) and remains constant:
...
function SimOptDecisions.run_timestep(
    ...
    # Sample annual maximum surge at the gauge (relative to gauge zero)
    surge_dist = GeneralizedExtremeValue(sow.gev_μ, sow.gev_σ, sow.gev_ξ)
    surge_at_gauge = rand(rng, surge_dist)

    # Convert surge to water level relative to reference datum
    # surge_at_gauge is measured relative to gauge zero
    water_level = config.gauge_height_above_ref + surge_at_gauge + state.msl

    # House floor level (with elevation) relative to reference datum
    house_floor_level = config.house_height_above_ref + action.elevation_ft

    # Flood depth at house (positive = flooding)
    ...
    # MSL unchanged in this example (constant over time)
...
1. **Stationary storm climate**: We sample from a fixed GEV distribution each year. In reality, changing storm patterns would shift this distribution over time. The framework supports time-varying SOWs via `TimeSeriesParameter` if you want to model non-stationarity.

2. **Constant sea level**: We track MSL as state but keep it constant at the reference datum. A more realistic model could sample sea-level rise trajectories in the SOW and update MSL each timestep—the state structure already supports this extension.

3. **Independent annual damages**: The model assumes flood damage in one year doesn't affect vulnerability in future years. For a more realistic model, you might track cumulative damage or recovery state.

4. **Single decision point**: We choose elevation once at $t=0$. A more complex model could allow staged investments or adaptive policies that respond to observed flooding.

5. **Simplified cost model**: The elevation cost function uses USACE estimates but doesn't account for site-specific factors, permitting delays, or temporary relocation costs that vary by household.
