...
The policy has two parameters that work together to determine the elevation:

1. **`elevate_gate`** (0–1): A soft binary choice. If > 0.5, the house is elevated; otherwise it stays at ground level.
2. **`elevation_amount`** (0–14 ft): How high to elevate if we choose to elevate.

This two-parameter formulation lets the optimizer discover that "no elevation" may be optimal for some objectives, without requiring a separate policy type.

```{julia}
#| output: false
struct ElevationPolicy{T<:AbstractFloat} <: AbstractPolicy
    elevate_gate::T      # in (0, 1): if > 0.5 then elevate, else don't
    elevation_amount::T  # feet above current ground level (0-14)
end

# For optimization, we need to define how to convert between policy and parameter vector
ElevationPolicy(params::AbstractVector) = ElevationPolicy(params[1], params[2])
SimOptDecisions.params(p::ElevationPolicy) = [p.elevate_gate, p.elevation_amount]
SimOptDecisions.param_bounds(::Type{<:ElevationPolicy}) = [(0.0, 1.0), (0.0, 14.0)]
```

```{julia}
#| output: false
# Define the action: elevation is gated by the binary decision parameter
function SimOptDecisions.get_action(
    ...
    elevation = policy.elevate_gate > 0.5 ? policy.elevation_amount : 0.0
    return ElevationAction(elevation)
end
```
...
for elev in elevations
    # For elev=0, use gate < 0.5 (no elevation); for elev > 0, use gate > 0.5
    gate = elev == 0 ? 0.0 : 1.0
    policy = ElevationPolicy(gate, Float64(elev))
...
```{julia}
#| label: fig-pareto
#| fig-cap: "Pareto front showing trade-off between construction cost and expected NPV damages."
#| code-fold: true
let
    fig = Figure(; size=(700, 500))
    ax = Axis(
