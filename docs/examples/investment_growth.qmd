...
outcomes = [simulate(params, sow, policy) for sow in sows]
...
1. **Type definitions**: `AbstractConfig`, `AbstractSOW`, `AbstractPolicy` with parametric types
2. **TimeStepping interface**: `initialize`, `run_timestep`, `time_axis`, `finalize`
3. **Simulation**: Running single and multi-SOW simulations
4. **Optimization**: Using `metric_calculator(outcomes)` to aggregate results
