...
```{julia}
#| output: false
function SimOptDecisions.time_axis(config::InvestmentConfig, ::InvestmentSOW)
...
# All solutions on the Pareto front are equally optimal (no single "best")
# Here we just pick one at random to display
idx = rand(1:length(opt_result.pareto_params))
params = opt_result.pareto_params[idx]
objectives = opt_result.pareto_objectives[idx]

println("Invest fraction: $(round(params[1], digits=3))")
println("Expected value: \$$(round(objectives[1], digits=2))")
```
