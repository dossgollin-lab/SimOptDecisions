...
The optimizer needs two methods:
...
`params(policy)` (extracting values from an instance) is auto-derived from `ContinuousParameter` fields defined by `@policydef`.
...
With `@policydef`, `params()` is auto-derived from `ContinuousParameter` fields.
You need to add a vector constructor and type-level `param_bounds`:
