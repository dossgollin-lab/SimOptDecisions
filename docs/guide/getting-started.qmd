...
The optimizer needs a **vector constructor** for the policy type:

```julia
MyPolicy(params::AbstractVector) = MyPolicy(threshold=params[1])
```

`params(policy)` and `param_bounds(policy)` are **auto-derived** from `ContinuousParameter` fields defined by `@policydef`.
You can also pass explicit `bounds` to `optimize()`.
...
With `@policydef`, bounds are stored in `ContinuousParameter` fields and `params()`/`param_bounds()` are auto-derived.
You only need to add a vector constructor:
...
# Vector constructor (required for optimization)
OptimizablePolicy(params::AbstractVector) = OptimizablePolicy(threshold=params[1])
```
