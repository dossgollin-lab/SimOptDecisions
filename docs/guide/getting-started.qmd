...
The optimizer needs one additional method:
...
Both `params(policy)` and `param_bounds(::Type{<:MyPolicy})` are auto-derived from bounded `@continuous` fields defined by `@policydef`.
...
With `@policydef`, both `params()` and `param_bounds(::Type)` are auto-derived from bounded `@continuous` fields.
